...
#endif
/*
public class NetEditorFormFile {
...
public class cxNetEditorDriver {
    ...
    public cxNetEditorDriver (String apiEndPoint, bool _verbose = false) {
    ...
    /*
        ...
        public async Task<ResT> RequestAsync<ResT> (HTTP_METHOD method, String apiName, object reqPacket) where ResT : cxNetPacket {
            ...
            if (request.result == UnityWebRequest.Result.ConnectionError) {
               ...
               // ResT respond = JsonUtility.FromJson<ResT> (res);
                ...
                if (parameters != null && parameters.fields.Count > 0) {
                    using (Stream requestStream = request.GetRequestStream ()) {
                        ...
                        foreach (KeyValuePair<string, object> pair in parameters.fields) {
                            ...
                            if (pair.Value is NetEditorFormFile) {
                                NetEditorFormFile file = pair.Value as NetEditorFormFile;
        ...
        */
    /*
        public async Task<ResT> PostMultipart<ResT> (string apiName, NetFormFields parameters) {
            String url = GetURL(apiName);

            List<IMultipartFormSection> requestData = new List<IMultipartFormSection> ();
            foreach (KeyValuePair<string, object> pair in parameters.fields) {
                IMultipartFormSection part = null;
                if (pair.Value is NetFormFile) {

                    var file = (NetFormFile) pair.Value;
    ...
    */
    ...
    public async Task<ResT> RequestAsync<ResT> (HTTP_METHOD method, String apiName, object reqPacket) where ResT : cxNetPacket {
        ...
        RequestCallback<ResT> (method, apiName, reqPacket, (res) => {
            result = res;
        }, (err) => {
            error = err;
        });

        return await Task.Run<ResT> (() => {
    ...
    public void RequestCallback<ResT> (HTTP_METHOD method, String apiName, object reqPacket, Action<ResT> onResponse, Action<string> onError) where ResT : cxNetPacket {
        ...
        StartTask (SendRequest<ResT> (request, (response) => {
            onResponse (response);
        }, (error) => {
            onError (error);
        }));
    ...
    public async Task<ResT> PostMultipartAync<ResT> (string apiName, cxNetFormFields parameters) where ResT : cxNetPacket {
        ...
        PostMultipartCallback<ResT> (apiName, parameters, (res) => {
            result = res;
        }, (err) => {
            error = err;
        });
    ...
    IEnumerator SendRequest<ResT> (UnityWebRequest request, Action<ResT> onResponse, Action<string> onError) where ResT : cxNetPacket {
        ...
        if (request.result == UnityWebRequest.Result.ConnectionError) {
            ...
            string msg = request.downloadHandler.text ?? string.Empty;
            Debug.Log ($"RequestAsync Error:{request.error} msg:{msg}");
            onError ($"RequestAsync Error:{request.error} msg:{msg}");
        } else {
            if (verbose)
                Debug.LogFormat ("PostMultipart Response:{0}, Document:{1}", request.responseCode, request.downloadHandler.text);
...
#if UNITY_EDITOR
    private static void StartTask (IEnumerator update, Action end = null) {
...
#else 
    private static void StartTask (IEnumerator update, Action end = null) {
        Observable.FromCoroutine (s => update);
    }
#endif
}