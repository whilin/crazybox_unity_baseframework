...
public abstract class cxResourceBuilder {
    ...
    [MenuItem ("Assets/G-Tech Lab/Build ResourceBundle ", true, 0)]
    ...
    [MenuItem ("Assets/G-Tech Lab/Build Resource Bundle", false, 0)]
    ...
    [MenuItem ("Tools/G-Tech Lab/Build All Resource Bundle")]
    public static void BuildAllResource () {
        try {
            var defs = AssetDatabase.FindAssets ("t:cxResourceDefinination");
...
*/
    ...
    static void UploadTest () {
        var pkg = "//com.test.jongok.cxpackage";
        UploadPackage ("com.test.jongok", pkg);
    }

    public static void UploadPackage (string resourceId, string pkgFile) {
        var env = cxCreatorEnvironment.GetEnvironment ();

        cxResourcePackageStandardRepository repo = new cxResourcePackageStandardRepository (env.ResourceUploadServerUrl, null);
