...
[CustomEditor(typeof(cxFxPlayer))]
[CanEditMultipleObjects]
public class cxFxPlayerEditor : cxInspectorEditor
{
    ...
    void ListBuild()
    {
        ...
        // m_compOrder = new ReorderableList(player.m_composerList, typeof(cxFxPlayer.FxComposer));

        m_compOrder = new ReorderableList(serializedObject, m_compListProp, true, true, true, true);//(player.m_composerList, typeof(cxFxPlayer.FxComposer));
        ...
        for (int i = 0; i < player.m_composerList.Length; i++)
        {
            ...
            var list = new ReorderableList(comp.fxList, typeof(cxFxPlayer.FxComposer));
    ...
    cxFxPlayer.FxComposer m_selectedFx;
    ...
    void DrawFxComposer(cxFxPlayer.FxComposer fx)
    {
        //fx.composerName =  EditorGUILayout.TextField("Composer Name", fx.composerName, GUILayout.Width(240));
        //EditorGUILayout.ObjectField(m_selectedProp);//(fx, typeof(cxFxPlayer.FxComposer));
        ...
        {
            fx.fxList.Add(new cxFxPlayer.FxUnit());
        }
    }

    void DrawFxUnit(cxFxPlayer.FxComposer fx, cxFxPlayer.FxUnit fxUnit)
    {
        EditorGUILayout.BeginVertical("Box");

        fxUnit.fxType = (cxFxPlayer.FxType)EditorGUILayout.EnumPopup("FxType", fxUnit.fxType);
        fxUnit.delay = EditorGUILayout.FloatField("Delay", fxUnit.delay);

        if (fxUnit.fxType == cxFxPlayer.FxType.Particle ||
            fxUnit.fxType == cxFxPlayer.FxType.TextMesh ||
            fxUnit.fxType == cxFxPlayer.FxType.Object)
        {
            ...
            fxUnit.emitDir = (cxFxPlayer.EmitDir)EditorGUILayout.EnumPopup("EmitDir", fxUnit.emitDir);
        }

        if (fxUnit.fxType == cxFxPlayer.FxType.Particle|| fxUnit.fxType == cxFxPlayer.FxType.Object)
        {
            fxUnit.prefabFx = EditorGUILayout.ObjectField("Particle Prefab", fxUnit.prefabFx, typeof(GameObject), false) as GameObject;
        }

        if (fxUnit.fxType == cxFxPlayer.FxType.TextMesh)
        {
        ...
        // if (fxUnit.fxType == cxFxPlayer.FxType.HapticFx)
        //{
        //    fxUnit.hapticFx = (afHapticFxType) EditorGUILayout.EnumPopup("Haptic Fx", fxUnit.hapticFx);
        //}


        if (fxUnit.fxType == cxFxPlayer.FxType.AnimationPlay)
        {
            ...
            {
                string[] aniList = null;
                cxGameObjectUtil.FindAniClips(fxUnit.animator.runtimeAnimatorController, 0, ref aniList);
        ...
        if (fxUnit.fxType == cxFxPlayer.FxType.Active || fxUnit.fxType == cxFxPlayer.FxType.Inactive)
            fxUnit.activeTarget = EditorGUILayout.ObjectField("Active Target", fxUnit.activeTarget, typeof(GameObject), true) as GameObject;

        if (fxUnit.fxType == cxFxPlayer.FxType.EventCall)
       {
